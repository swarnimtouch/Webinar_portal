                                        ...
                                        @if(auth()->user()->type === 'admin')
                                            <div class="mb-10">
                                                ...
                                                <select class="form-select form-select-solid fw-bolder"
                                                    ...
                                                    @foreach($banner as $b)
                                                        <option value="{{ $b->event_id }}">
                                                            {{ optional($b->event)->name ?? 'N/A' }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
    ...
    <script>
            ...
            bannerTable = $('#kt_table_banners').DataTable({
                ...
                ajax: {
                    ...
                    data: d => {
                        ...
                        if (isAdmin) d.event = qs('[data-kt-user-table-filter="event"]').value;
                    }
                },
                order: [[2, 'asc']],
                pageLength: 10,
                drawCallback: function () {
                    document.querySelectorAll('#kt_table_banners td video').forEach(video => {
                            ...
                            row.type === 'image'
                               ...
                               <div style="width:80px;height:80px;border-radius:4px;overflow:hidden;position:relative;background:#000;">
                                   ...
                                   <div class="play-overlay"
                                        style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;transition:opacity 0.2s;">
        ...
        document.addEventListener('click', e => {
            ...
            if (videoLink) {
                ...
                const videoUrl = videoLink.dataset.url;
        ...
        document.addEventListener('mouseenter', e => {
            const video = e.target.closest('#kt_table_banners td video');
            if (!video || video.id === 'modalVideoPlayer') return;
            ...
            const overlay = video.closest('.video-link')?.querySelector('.play-overlay');
        ...
        qs('[data-kt-user-table-filter="reset"]').addEventListener('click', () => {
            const typeSelect = qs('[data-kt-user-table-filter="type"]');
            if (typeSelect) {
                typeSelect.value = '';
        ...
        document.addEventListener('change', e => {
            if (!e.target.classList.contains('row-checkbox') && !e.target.matches('[data-kt-check="true"]')) return;
            ...
            qsa('.banner-status-toggle').forEach(t => t.disabled = count > 1);
        });

        KTUtil.onDOMContentLoaded(() => {
            initBannerTable();
            $('#kt_table_banners').on('draw.dt', () => {
                if (typeof $.fn.viewbox === 'function') $('.image-link').viewbox();
            });
        });
    </script>
@endpush
